...
class UploadHandler
{
    ...
    public function saveImage($object_id, UploadedFile $file, $folder, $editor, $file_prefix, $max_width = false)
    {
        $type = 'image';
        // 构建存储的文件夹规则，值如：uploads/images/avatars/201709/21/
        // 文件夹切割能让查找效率更高。
        $folder_name = "public/images/$folder/" . date("Ym", time()) . '/'.date("d", time());
...
//        $filename = $file_prefix . '_' . time() . '_' . str_random(10) . '.' . $extension;
    ...
    public function reduceSize($image, $max_width)
    {
        ...
        return [
            'data' => $image->encode(pathinfo($image->basePath(), PATHINFO_EXTENSION)),
            'image' => $image
        ];
    }
