<template>
    <section>
        <el-main>
            <table-filter @filter="filter" @delRecords="showDelete = true" :exactTypes="exactTypes"
                          layout="del, exact, input, search"></table-filter>
            ...
            <el-table :data="list" stripe style="width: 100%" @selection-change="selsChange">
                ...
                <el-table-column prop="article.title" label="文章名"></el-table-column>
...
<script>
    ...
    import { apiGetArticleComments, apiDeleteArticleComment } from '../../api/backend.js';
    ...
    export default {
        ...
        methods: {
            ...
            getList(keyword) {
                ...
                let self = this;
                apiGetArticleComments(params).then(res => {
                    if (res.code === 1) {
            ...
            delRecord() {
                let self = this;
                apiDeleteArticleComment({ids: this.ids}).then( res => {
                    if (res.code === 1) {
